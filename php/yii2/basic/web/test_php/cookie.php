...
<?php
/*
 * cookie 客户在第一次请求test.php的时候，setcookie会在客户端浏览器中种下cookie（在response的时候）， * 那么当第二次请求的时候浏览器会在头信息中自动包含该域下未过期的cookie发送给server,正式因为这个原因在第一次访问的时候并获取不到cookie，第二次访问的时候才能获取到，因为第一次只是服务器端向客户端注册cookie，request的头信息中并没有cookie，第二次就出现了。
