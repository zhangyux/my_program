 ...
 * <p/>
 * Using this object, you can get/create/update/delete documents. You can also call views (named and adhoc) to query the
 * underlying database.
 *
 * @author mbreese
 */
public class Database {
	...
	/**
	 * The number of documents in the database <b>at the time that it was retrieved from the session</b> This number
	 * probably isn't accurate after the initial load... so if you want an accurate assessment, call
	...
	/**
	 * The update seq from the initial database load. The update sequence is the 'revision id' of an entire database.
	 * Useful for getting all documents in a database since a certain revision
	 *
	 * @return
	 * @see getAllDocuments()
	 */
	...
	/**
	 * Runs a named view on the database This will run a view and apply any filtering that is requested (reverse,
	 * startkey, etc).
	 *
	 * @param view
	 * @return
	...
	/**
	 * Runs an ad-hoc view from an AdHocView object. You probably won't use this much, unless you want to add filtering
	 * to the view (reverse, startkey, etc...)
	 *
	 * @param view
	 * @return
	 */
	public ViewResults adhoc(final AdHocView view) {
		...
		String adHocBody = new JSONStringer().object().key("map")
				.value(JSONUtils.stringSerializedFunction(view.getFunction())).endObject().toString();
	...
	/**
	 * Retrieves a document from the database and asks for a list of it's revisions. The list of revision keys can be
	 * retrieved from Document.getRevisions();
	 *
	 * @param id
	 * @return
	 */
	...
	/**
	 * Update an existing document using a document update handler. Returns false if there is a failure making the
	 * PUT/POST or there is a problem with the CouchResponse.
	 *
	 * @author rwilson
	 * @param update
	 * @return
	...
	public boolean updateDocument(Update update) {
		...
		url = this.name + "/" + ((elements.length < 2) ? elements[0] : DESIGN + elements[0] + UPDATE + elements[1])
				+ "/" + update.getDocId();

		if (update.getMethodPOST()) {
			try {
				// Invoke the POST method passing the parameters in the body
				CouchResponse resp = session.post(url, "application/x-www-form-urlencoded",
						update.getURLFormEncodedString(), null);
				return resp.isOk();
			} catch (Exception e) {
				return false;
			}
	...
	/**
	 * Update an existing document using a document update handler and return the message body. Returns null if the is
	 * problem with the PUT/POST or CouchResponse.
	 *
	 * @author rwilson
	 * @param update
	 * @return
